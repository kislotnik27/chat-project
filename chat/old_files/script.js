...
function displayMessage(message) {
    ...
    messageElement.classList.add(message.sender); // Добавляем класс на основе отправителя
    let messageContent;
    ...
    if (message.reply_to_message_id) {
        ...
        fetch(`get_message_by_id.php?message_id_tg=${message.reply_to_message_id}`)
            .then(response => response.json())
            .then(replyMessageData => {
                if (replyMessageData.status === 'success') {
                    ...
                    repliedMessageElement.innerHTML = `<strong>Manager</strong> ${repliedMessage.message}`;
